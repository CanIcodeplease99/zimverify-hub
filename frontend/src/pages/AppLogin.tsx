...
const AppLogin = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const { error, needsConfirmation } = await signUpWithEmail(email, password, fullName, "public");
            ...
            <>
              ...
              <form onSubmit={handleDemoLogin} className="space-y-5">
                <div>
                  ...
                  <Input type="email" className="mt-1.5 h-12 rounded-xl border-border/60 bg-card/50" value="demo@zimverify.gov.zw" readOnly data-testid="demo-email-input" />
                </div>
                <div>
                  <Label className="font-body text-sm font-medium">Password</Label>
                  <Input type="password" className="mt-1.5 h-12 rounded-xl border-border/60 bg-card/50" value="demo1234" readOnly data-testid="demo-password-input" />
                </div>
              ...
              <p className="text-xs text-muted-foreground font-body mt-4 text-center">Demo mode uses mock data. Switch to "Live Login" for real authentication.</p>
            </>
          ) : (
            <>
              ...
              <form onSubmit={isSignUp ? handleSignUp : handleSupabaseLogin} className="space-y-5">
                {isSignUp && (
                  <div>
                    ...
                    <Input type="text" placeholder="Your full name" value={fullName} onChange={e => setFullName(e.target.value)} className="mt-1.5 h-12 rounded-xl border-border/60 bg-card/50" data-testid="signup-name-input" />
                  </div>
                )}
                <div>
                  <Label className="font-body text-sm font-medium">Email</Label>
                  <Input type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} className="mt-1.5 h-12 rounded-xl border-border/60 bg-card/50" data-testid="email-input" />
                </div>
                <div>
                  <Label className="font-body text-sm font-medium">Password</Label>
          ...
          <div className="flex flex-wrap gap-2 mt-6">
            {[{ label: "Public Access" }, { label: "Vehicle Checks" }, { label: "Secure" }].map(chip => (
              <Badge key={chip.label} variant="secondary" className="font-body text-xs rounded-lg px-3 py-1 bg-card/80 border border-border/40">
                <Shield className="h-3 w-3 mr-1" />{chip.label}
              </Badge>
            ))}
          </div>

          <div className="mt-8 pt-6 border-t border-border/30 text-center">
            <p className="text-sm text-muted-foreground font-body mb-2">Government, Police, or Customs officer?</p>
            <Button variant="outline" onClick={() => navigate("/app/official/login")} className="font-body rounded-xl border-border/40" data-testid="go-to-official-login">
              <Shield className="mr-2 h-4 w-4" /> Official Portal Login
            </Button>
          </div>
        </motion.div>
      </div>
      ...
      <div className="hidden lg:block relative overflow-hidden">
        ...
        <div className="relative z-10 h-full flex items-center justify-center p-16">
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="max-w-md space-y-6">
            ...
            <p className="text-white/80 font-body text-lg leading-relaxed">Check any vehicle's registration, ownership history, and legal status instantly. Trusted by 14 million Zimbabweans.</p>
            <div className="space-y-3">
              {["Instant plate & VIN lookups", "Ownership verification reports", "Stolen vehicle alerts", "Digital receipts & history"].map(item => (
                <div key={item} className="flex items-center gap-3 text-white/90 font-body">
