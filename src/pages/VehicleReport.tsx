    ...
    <div className="space-y-8 max-w-4xl">
      ...
      {selectedCheck === "full" && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex flex-wrap gap-3 items-center">
          ...
          {bundles.map((b) => (
            <Badge key={b.id} variant="outline" className="border-accent/20 bg-accent/5 text-foreground font-body text-xs">
      ...
      {searched && !loading && (
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-5">
          ...
          {/* Full report sections — only for full check */}
          <AnimatePresence>
            {!isQuick && (
              <>
                {/* Ownership */}
