      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          {stakeholders.map((item, i) => (
            ...
            >
              <div className="relative h-[460px] rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-500">
                <img
                  src={item.image}
                  alt={item.alt}
        ...
        >
          {[
            { tag: "Claims & risk", title: "Insurance Partners", desc: "Claims verification, fraud detection, risk assessment, and batch processing for insurers across Zimbabwe." },
            { tag: "API & Integration", title: "Commercial Partners", desc: "API access, bulk uploads, fleet verification, token management, and usage analytics for dealers and finance houses." },
          ].map((item) => (
            <div key={item.title} className="p-8 rounded-2xl border border-border/60 bg-card/60 backdrop-blur-sm hover:border-primary/30 hover:bg-card hover:shadow-lg transition-all duration-500 hover:-translate-y-1">
